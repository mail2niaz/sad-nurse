	...
	$.fn.multiDatesPicker = function(method) {
		...
		var methods = {
			...
			addDates : function( dates, type ) {
				if(dates.length > 0) {
					...
					switch(typeof dates) {
						...
						/* Insert */
							actiondate = "ins";
							date_org = dateConvert.call(this, dates);
							curdate = Date.parse(date_org);
							var cdate = (parseInt(date_org.getMonth()) + 1);
							var clength = cdate.toString().length;
							if(clength == '1'){ cmonth = '0'+cdate; }else if(clength == '2'){ cmonth = cdate; }
							jQuery('#cmonth').val(cmonth);
							$('#sel_date').val(curdate);
							$('#action').val(actiondate);
							var $about = $("#edit_container");
							$about.show();
							$about.dialog({
					         title: "",
					         width: 600,
			...
			removeDates : function( dates, type ) {
				...
				jQuery('#cmonth').val(cmonth);
				 actiondate = "rd";
				 var url = $('.box').attr('valoid')+"/"+curdate+"/"+actiondate;
				 jQuery.ajax({
				 type: "POST",
				 url: url,
				 success: function(msg)
				 {
				 	var optsiteurl = jQuery('#optsiteurl').val();
