...
<script type="text/javascript">
	...
	var $about = $("#event_edit_container");
				 ...
				 function copy_reassign (week,year,oid, jobdate, section, type) {
		$('#LoadingImage').show();
	var $about = $("#event_edit_container");
				  $.post("<?php echo site_url($i18n.'jobassign/copy_reassign_job') ?>/"+week+"/"+year+"/"+oid+"/"+jobdate+"/"+section+"/"+type,
				  function (data) {
				  	$('#LoadingImage').hide();
				       $("#event_edit_container").html(data);
				       $about.dialog({
				         title: "<?php echo ( sprintf( lang("JOBASSIGN::copy_job")) ); ?>",
				         width: 900,
				         top: 0,
				         closeOnEscape: true,
				         position:'center',
	...
	var $about = $("#event_edit_container");
		...
		<div class='flash' style="display: none; float: left; padding-left: 350px;">settimana copiata con successo</div>
	
        <input type="hidden" name="siteurl_pdf" value="<?php echo site_url($i18n.'jobassign_filter/create_pdf') ?>" id = "siteurl_pdf"/>
	<div id='calendar1'>
		...
		<!--<a href="<?php echo site_url($i18n.'jobassign_filter/create_pdf/0/0/0/0/'.$week."/".$year) ?>" class="btn btn-rounded btn-primary btn-submit"><?php echo lang("JOBASSIGN-REPORT::export_pdf"); ?></a>-->
		<a href="#" onclick="pdf_ajax(0,0,0,0,<?php echo $week;?>,<?php echo $year;?>)" class="btn btn-rounded btn-primary btn-submit"><?php echo lang("JOBASSIGN-REPORT::export_pdf"); ?></a>
...
<div class="scrollbar">
<select name="weekno" id="weekno_navgation" onchange="return navigation_select('0','0','0','0','0','0');">
...
?>
<select name="weekno_navgation" id="weekno_navgation" onchange="return navigation_select('0','0','0','0','0','0')">
<option value="" selected="selected"> </option>
<?php foreach($start as $key=>$value){ ?>
<option value="<?=$key?>#<?=$year?>" ><?php echo $value['start']." - ".$value['end']; ?></option>
<?php } 
*/
?>
<?php
...
?>
		<a href="javascript:void(0)" class="btn btn-rounded btn-primary btn-submit" onclick="return navigation('<?=$back_weeknumber?>','<?=$year_week_back?>','0','0','0','0')"><?php echo lang("JOBASSIGN::prev"); ?></a>
...
<td class="wc-day-column day-<?=$d?> morning">
	<div class="wc-day-column-inner ui-droppable schedule">
						<div class="wc-cal-event ui-corner-all">
							...
							<i class="icon-copy" onclick="copy_reassign('<?=$week?>','<?=$year?>','<?=$oid?>', '<?=$jobdate?>','1')"></i>
			<i class="icon-share" onclick="reassign('<?=$week?>','<?=$year?>','<?=$oid?>', '<?=$jobdate?>','1')"></i><i class="icon-edit" onclick="job_maintain('<?=$week?>','<?=$year?>','<?=$oid?>', '<?=$jobdate?>')"></i>
							</div></div>
...
<td class="wc-day-column day-<?=$yd?> morning">
	<div class="wc-day-column-inner ui-droppable schedule">
						<div class="wc-cal-event ui-corner-all" style="background-color: #FFFFFF;">
							...
							<i class="icon-share" onclick="reassign('<?=$week?>','<?=$year?>','<?=$yellow_oid?>', '<?=$jobdate_yellow?>','1','y')"></i>
							</div></div>
					...
					<div class="wc-day-column-inner ui-droppable schedule">
						<div class="wc-cal-event ui-corner-all" style="background-color: #FFFFFF;">
							<div class="wc-time ui-corner-all"><span><?=lang("OPERATOR::afternoon")?></span>
<div style="float: right; margin-right: 10px;">
                                                       	<i class="icon-share" onclick="reassign('<?=$week?>','<?=$year?>','<?=$yellow_oid?>', '<?=$jobdate_yellow?>','2','y')"></i>
							</div></div>
							<div class="wc-title" title="">
								<ul class="jobpatientlist">
									<?php
...
/* Job maintain js */
function job_maintain (week,year,oid,jobdate) { 
	$('#LoadingImage').show();
	var $about = $("#event_edit_container");
	$.post("<?php echo site_url($i18n.'jobassign/job_maintain_popup') ?>/"+week+"/"+year+"/"+oid+"/"+jobdate,
	function (data) {
		$('#LoadingImage').hide();
		$("#event_edit_container").html(data);
		$about.dialog({
			title: "<?php echo ( sprintf( lang("JOBASSIGN::job_maintain_popup")) ); ?>",
			width: 900,
			top: 0,
			closeOnEscape: true,
			position:'center',
...
/* Job After noon maintain js */
function job_afternoon_maintain (week,year,oid,jobdate) { 
	$('#LoadingImage').show();
	var $about = $("#event_edit_container");
	$.post("<?php echo site_url($i18n.'jobassign/job_after_noon_maintain_popup') ?>/"+week+"/"+year+"/"+oid+"/"+jobdate,
	function (data) {
		$('#LoadingImage').hide();
		$("#event_edit_container").html(data);
		$about.dialog({
			title: "<?php echo ( sprintf( lang("JOBASSIGN::job_after_noon_maintain_popup")) ); ?>",
			width: 900,
			top: 0,
			closeOnEscape: true,
			position:'center',
...
/* End Job After noon maintain js */
...
function navigation(week,year,pid,filt_oid,dist_id,filter_box_status) {
	...
	function navigation_select(week_nav,year_nav,pid,filt_oid,dist_id,filter_box_status) {
               var week = jQuery('#weekno_navgation').val( );
               var split_week = week.split('#');
		...
		jQuery.post(siteurl,
               ...
               //	alert(data);
               	$('#field_loader').hide();
               jQuery('select option[value="48#2014"]').attr("selected",true);
               	if(data != ''){
               		jQuery('#calendar1').html('');
               		jQuery('#calendar1').html(data);
