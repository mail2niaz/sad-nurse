...
<?php echo anchor($this->lang->mci_current().'/report/external_webservice_export/'.$pid."/".$oid."/".$from_date."/".$to_date."/".$did,lang("REPORT::export_csv"),array('class' => 'btn btn-primary btn-submit export')); ?>
...
<?php
   ...
   public function external_webservice_export($pid = NULL, $oid = NULL, $from_date = NULL ,$to_date = NULL, $did = NULL){
      ...
      $result_web =  $this->report_model->getwebserviceurl();
      $TPD2009 =  $result_web['url'];
      $platform_code = $result_web['platform_code'];
 	...
 	foreach($export_P2000 as $result){
            ...
            $sel_job = $this->db->get_where("assign_job_list",array("aid" => $aid));
            $fet_job = $sel_job->row_array();
		...
		foreach($result_id as $results){ // Begin of operator code array
			if($oid == $pry_oid) {
				$p200_code_op = $results['code_op1'];
			}else if($oid == $sec_oid) {
				$p200_code_op = $results['code_op2'];
			}else if($oid == $sup_id) {
				$p200_code_op = $results['code_op3'];
			} else {
				$p200_code_op = "0";
			}
			/*Get Patient ssn */
			$sel = $this->db->get_where("patients",array("pid" => $pid));
                  $fet = $sel->row_array();
			$ssn = $fet['ssn'];
			$ws_code = $fet['ws_code'];
			$sel_contract = $this->db->get_where("contract_details",array("pid" => $pid));
                  $fet_contract = $sel_contract->row_array();
			$cid = $fet_contract['cid'];
	
			/*Get Operator ssn */
			$sel_opertor = $this->db->get_where("operators",array("oid" => $oid));
                  $fet_operator = $sel_opertor->row_array();
			$operator_ssn = $fet_operator['ssn'];
					  
		if($i == 1) // First job condition
		{
			$evento[] =  array(
							"idEventoTrasmesso" => $platform_code.'_'.$ped_id.'_'.'A',
							"codiceContratto" => $platform_code,
							"codiceSottoContratto" => $ws_code,
							"codicePresidio" => '',
			...
			$evento[] = array(
				...
				$i++;	
	} // End of operator code array
			
			  
                        foreach ($evento as $event) {  // Begin of an event array
                        
                              
                             // $insert_send_data = $this->report_model->senddata_webservice_insert($event,$response_arr);
