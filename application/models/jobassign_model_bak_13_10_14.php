...
if($action == "new"){
	if(isset($_REQUEST['pry_operator'])){
		if($_REQUEST['pry_operator'] != ""){
			mysql_query("INSERT INTO `assign_job_status` (`aid`, `oid`, `opt_type`) VALUES ('$aid', '$_REQUEST[pry_operator]', '1')");
		}
	}
if(isset($_REQUEST['sec_operator'])){
		if($_REQUEST['sec_operator'] != ""){
			mysql_query("INSERT INTO `assign_job_status` (`aid`, `oid`, `opt_type`) VALUES ('$aid', '$_REQUEST[sec_operator]', '2')");
		}
	}
if(isset($_REQUEST['sup_operator'])){
		if($_REQUEST['sup_operator'] != ""){
			mysql_query("INSERT INTO `assign_job_status` (`aid`, `oid`, `opt_type`) VALUES ('$aid', '$_REQUEST[sup_operator]', '3')");
		}
	}
}elseif($action == "edit"){
	if(isset($_REQUEST['pry_operator'])){
	if(isset($_REQUEST['default_pry_operator'])){
		if($_REQUEST['default_pry_operator'] != $_REQUEST['pry_operator']){
			if($_REQUEST['pry_operator'] != ""){
				mysql_query("INSERT INTO `assign_job_status` (`aid`, `oid`, `opt_type`) VALUES ('$uaid', '$_REQUEST[pry_operator]', '1') ON DUPLICATE KEY UPDATE `aid` = $uaid, `oid` = values(oid), `opt_type` = values(opt_type)");
			}
	} }else{
		if($_REQUEST['pry_operator'] != ""){
		mysql_query("INSERT INTO `assign_job_status` (`aid`, `oid`, `opt_type`) VALUES ('$uaid', '$_REQUEST[pry_operator]', '1') ON DUPLICATE KEY UPDATE `aid` = $uaid, `oid` = values(oid), `opt_type` = values(opt_type)"); }
	...
	if(isset($_REQUEST['default_sec_operator'])){
		if($_REQUEST['default_sec_operator'] != $_REQUEST['sec_operator']){
			if($_REQUEST['sec_operator'] != ""){
				mysql_query("INSERT INTO `assign_job_status` (`aid`, `oid`, `opt_type`) VALUES ('$uaid', '$_REQUEST[sec_operator]', '2') ON DUPLICATE KEY UPDATE `aid` = $uaid, `oid` = values(oid), `opt_type` = values(opt_type)");
			}
	} }else{
		if($_REQUEST['sec_operator'] != ""){
				mysql_query("INSERT INTO `assign_job_status` (`aid`, `oid`, `opt_type`) VALUES ('$uaid', '$_REQUEST[sec_operator]', '2') ON DUPLICATE KEY UPDATE  `aid` = $uaid, `oid` = values(oid), `opt_type` = values(opt_type)"); }
...
mysql_query("UPDATE assign_job_list SET $field = '$reassign_opt' WHERE $oid IN (pry_oid,sec_oid,sup_id) AND patient_id = '$pid' AND job_date_assign = '$job_assign_date'");
...
public function select_updated_opt_list()
{
 ...
 $sel1 = "SELECT oid FROM `operator_working_days` where (leavetime = '$leavetime' AND FROM_UNIXTIME(SUBSTRING(wrk_date, 1, LENGTH(wrk_date)-3), '%Y-%m-%d') = '$job_date_assign') OR (FROM_UNIXTIME(SUBSTRING(wrk_date, 1, LENGTH(wrk_date)-3), '%Y-%m-%d') = '$job_date_assign' AND leavetime = '3')";
	$sel = "select oid,username,firstname,lastname,role from operators where role IN ($role) AND status = '1' $qry_tag AND oid NOT IN($sel1) order by lastname ASC";
...
public function putcopyweekdata()
{
	...
	$error = '';
	//$sel = "SELECT * FROM assign_job_list WHERE week = '$selectedweekno' AND year = '$year'";
	$sel = "SELECT b.* FROM operators as a,assign_job_list as b WHERE b.week = '$selectedweekno' AND b.year = '$year' AND FIND_IN_SET('$district', a.dist_id) AND a.oid IN (pry_oid,sec_oid,sup_id)";
	$qry = mysql_query($sel);
	$cnt = array();
	$fetdata = array();
	$i = 0;
	...
	while($fet = mysql_fetch_assoc($qry)){
		...
		$check_sel = mysql_query("SELECT b.* FROM operators as a,assign_job_list as b where b.job_date_assign = '$jobdate' AND b.request_id = '$request_id' AND FIND_IN_SET('$district', a.dist_id) AND -a.oid IN (pry_oid,sec_oid,sup_id)");
		...
		if((strpos($minus_fet['pry_oid'],'-') !== false) || (strpos($minus_fet['sec_oid'],'-') !== false) || (strpos($minus_fet['sup_id'],'-') !== false)){
			...
			if($qry_fet_contract['last_ceased_date'] != '0'){
				...
				if($weekno < $contweek){
					mysql_query("INSERT INTO assign_job_list (request_id,contract_int_id,intervent_type_id,patient_id,pry_oid,sec_oid,sup_id,cid,start_time_hour,start_time_min,end_time_hour,end_time_min,sel_week_day,week,year,job_date_assign) SELECT request_id,contract_int_id,intervent_type_id,
		patient_id,-pry_oid,-sec_oid,-sup_id,cid,start_time_hour,start_time_min,end_time_hour,end_time_min,sel_week_day,$weekno,$year,'$jobdate' FROM assign_job_list where aid = '$aid'");
				}else{
$error .= "yes";
				}
			}else{
...
public function getweekfortnightly($report_type, $weekno, $district)
{
	if($district == '0'){
		$did = '';
	}else{
	...
	if($report_type == '1'){
		$sel = $this->db->query("select a.pid, sum(a.total_weekdays) as total_contract, (select count(aid) from assign_job_list where patient_id = a.pid AND week = '$weekno') as planned_job, (sum(a.total_weekdays) - (select count(aid) from assign_job_list where patient_id = a.pid AND week = '$weekno')) as difference  from contract_intervent_weekdays as a INNER JOIN patients as b on (a.pid = b.pid) where a.week_days != '' $did group by a.pid order by b.surname");
	}elseif($report_type == '2'){
		$pweek = $weekno - 1;
		$sel = $this->db->query("select a.pid, (select count(aid) from assign_job_list where patient_id = a.pid AND week IN ('$pweek','$weekno')) as planned_job from contract_intervent_weekdays as a INNER JOIN patients as b on (a.pid = b.pid) where a.intervent_fortnightly = 'on' $did group by a.pid order by b.surname");
	}
	$val = $sel->result();
	return $val;
}
