...
$q = $this->db->query("SELECT DISTINCT AJ.aid, `P`.*, `A`.`request_id` as requestid, `A`.`intervent_type_id`, `O`.`role`, `A`.`start_time_hour`, `A`.`start_time_min`, `A`.`end_time_hour`, `A`.`end_time_min`,`A`.`job_date_assign`,`E`.`int_type` FROM (`assign_job_list` as A) JOIN `patients` as P ON `A`.`patient_id`= `P`.`pid` JOIN `operators` as O ON `A`.`pry_oid`= `O`.`oid` OR A.sec_oid= O.oid OR A.sup_id = O.oid JOIN `assign_job_status` as AJ ON `AJ`.`aid`= `A`.`aid` JOIN  `intervention_types` as E ON `E`.`int_type_id`= `A`.`intervent_type_id` WHERE `AJ`.`oid` = '$oid' AND (`A`.`job_date_assign` between '$td' AND '$sd') AND `AJ`.`status` = '1' ORDER BY `A`.job_date_assign ASC, TIME(CONCAT(`A`.start_time_hour,':', `A`.start_time_min)) ASC");
	$val = $q->result();
	return $val;
}
...
public function offline_save_data()
{
	...
	$request_id = $ary['request_id'];
	$int_type_id = $ary['int_type_id'];
	$sel_update = "UPDATE assign_job_status SET status='2', closed_datetime = '$time' WHERE aid = '$aid' AND oid = '$oid'";
      $succ = $this->db->query($sel_update);
	/* Time Details	 */
	$current_date = date("Y-m-d");
	$starttime = $ary['starttime'];
	$endtime = $ary['endtime'];
...
public function geo_location_insert($oid,$aid,$type,$latitude,$longitude) 
{
      $time = time();
      $data=array(
            'oid'=>$oid,
      ...
      $this->db->insert('operator_geo_location',$data);
}
}
?>
