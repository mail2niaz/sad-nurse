...
Class Intervent_model extends CI_Model
{
	 ...
	 private $tbl_job_operator_code = 'job_operator_code';
...
function getintertype_job_code($int_code){
		$this->db->where('id_job', $int_code);
		$result = $this->db->get($this->tbl_job_operator_code);
		return $result->result_array();
}
public function add_operator_code() {
      $time = time();
      $data=array(
            'id_job'=>$this->input->post('id_job'),
            'code_op1'=>$this->input->post('code_op1'),
            'code_op2'=>$this->input->post('code_op2'),
            'code_op3'=>$this->input->post('code_op3'),
            'last_modify'=>$time,
      );
       
      $this->db->insert($this->tbl_job_operator_code,$data);
      return mysql_insert_id();
      }
public function update_operator_code() {
      $time = time();
      ...
      $this->db->where('id',$id);
	$this->db->update($this->tbl_job_operator_code,$data);  
      return mysql_insert_id();
      }

function getintertype_job($intid_job){
            //echo $intid_job;die;
		$this->db->where('id', $intid_job);
		return $this->db->get($this->tbl_job_operator_code);
}
public function deleteinttype_job_code($job_id)
{
	$this->db->where('id',$job_id);
	$this->db->delete($this->tbl_job_operator_code);
}
}
?>
